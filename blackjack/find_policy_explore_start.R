...
player_round_explore <- function(x, y, a1, policy_player, ref){
  MDP <- NULL
  rand <- TRUE
  while (1){
    s <- count(x)
...
## play game (one episode with fixed start and exploring start) 
play_blackjack_explore <- function(s, y, ace, a1, policy_player, ref){
  ...
  if (out1!=2){
    ...
    ## player's turn
    out2 <- player_round_explore(x=x2, y=x1[1], a1=a1, policy_player, ref)
    x2 <- out2$x
    MDP <- out2$MDP
...
## evaluation function (with exploring start)
policy_eva_explore <- function(N, policy_player, ref, learning_rate){
  ...
  for (i in 1:N){
    ...
    a1 <- rbinom(1, 1, 0.5)
    episode <- play_blackjack_explore(s, y, ace, a1, 
...
policy_iter <- function(N=5e4, policy_player_init, visualize=FALSE, 
  ...
  ref0 <- policy_eva_explore(N, policy_player_init, NULL, NULL)
  ...
  for (ii in 1:M_iter){
    ...
    ref1 <- policy_eva_explore(N, policy_player_update, ref0, learning_rate)
